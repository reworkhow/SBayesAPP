...
#     Gprior_vec[c] = Gprior_vec[c] / nLoci_annot[c]
# end
...
function runMPI(; 
    ...
    deltaArray = [ones(my_nsnp * nCategory) for t in 1:nTraits]
    ...
    #mcmc_Alpha = [zeros(my_nsnp * nCategory, nOutput) for t in 1:nTraits]
    ...
    @showprogress "running MCMC ..." for iter = 1:nIter
        ...
        if iter > burnin
            varg_cat = sum(varg_blk_cat) # sum varg_blk_cat to get varg for different category & trait across all blocks
            varg_cov_cat = sum(varg_cov_blk_cat) # sum varg_cov_blk_cat to get genetic covariance for different category across all blocks
    ...
    #writedlm(analysis_path * "mcmc_Alpha1.rank$my_rank.txt", mcmc_Alpha[1])
    #writedlm(analysis_path * "mcmc_Alpha2.rank$my_rank.txt", mcmc_Alpha[2])
