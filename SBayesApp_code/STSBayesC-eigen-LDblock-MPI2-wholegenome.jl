...
function runMPI(; niter=nIter, outFreq=outFreq, seed=seed,
    ...
    vary=vary, pi=startPi, estimatePi=true,
    ...
    my_TransformedX_dict = JLD2.load(data_path * "nrank$cluster_size.eigen/bhatXsqrt2pq_GWASfreq/995Eigen/rank$my_rank.TransformedX_dict.jld2")["my_TransformedX_dict"]
    my_TransformedY_dict = JLD2.load(data_path * "nrank$cluster_size.eigen/bhatXsqrt2pq_GWASfreq/995Eigen/rank$my_rank.TransformedY_dict.jld2")["my_TransformedY_dict"]
    my_blkSNPsIndex_dict = JLD2.load(data_path * "nrank$cluster_size.eigen/bhatXsqrt2pq_GWASfreq/995Eigen/rank$my_rank.blkSNPsIndex_dict.jld2")["my_blkSNPsIndex_dict"]
    my_blkID = Int.(vec(readdlm(data_path * "nrank$cluster_size.eigen/bhatXsqrt2pq_GWASfreq/995Eigen/rank$my_rank.blkIDs.txt", ',')))  #block IDs for this rank
