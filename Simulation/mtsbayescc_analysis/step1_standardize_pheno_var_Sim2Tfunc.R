...
## Input data
##############################################################
# data_folder_path <- "/group/qtlchenggrp/jiayi/MTSBayesC/data/UKBioBank/Sim2Tfunc/"
# LD_info_path <- "/group/qtlchenggrp/jiayi/MTSBayesC/data/UKBioBank/GCTB_ldm/ukbEUR_HM3/"
# LDinfo_file <- "snp.info"
data_folder_path <- "/common/zhao/jyqqu/MTSBayesCC/data/sim_chr1_output_v2/"
LD_info_path <- "/common/zhao/tianjing/simu_chr1_output/"
LDinfo_file <- "snp.chr1.info"
##############################################################
## Input data
##############################################################
...
#data_folder_name <- paste0("rep", seed, "hsq", h21, "-", h22, "pleioPercent", pleio_percent, "_sampleSize", sample_size, "_annotationSize", annotation_size, "/")
#data_folder_name <- paste0("h2_trait1.", h21, ".h2_trait2.", h22, "_pleioPercent", pleio_percent, "_sampleSize", sample_size, "_annotationSize", annotation_size, "_seed", seed)
data_folder_name <- paste0("h2_trait1.", h21, ".h2_trait2.", h22, "_pleioPercent", pleio_percent, "_sampleSize", sample_size, "_seed", seed)
gwas_path <- paste0(data_folder_path, data_folder_name, "/") # /group/qtlchenggrp/jiayi/MTSBayesC/data/UKBioBank/RealData/T2D_Comorbidities/GWAS_summary_statistics/ImputedSumstat/
trait1_file <- "Trait1.gcta.phen.plink.ci.assoc.linear.ma" # T2D.imputed.ma
trait2_file <- "Trait2.gcta.phen.plink.ci.assoc.linear.ma" # SBP.imputed.ma
...
dir.create(paste0(gwas_path, "standardPhenoVar/"), recursive = TRUE)
write.csv(Trait1, file = paste0(gwas_path, "standardPhenoVar/b1_complete.txt"), row.names = FALSE)
write.csv(Trait2, file = paste0(gwas_path, "standardPhenoVar/b2_complete.txt"), row.names = FALSE)
